...
`)
        ...
        if (!sideArg && !amountArg) {
            const result = await (0, engine_1.resume)();
        ...
        if (side !== 'H' && side !== 'T') {
            throw errors_1.Errors.invalidSide(sideArg ?? '');
        }
        // Validate amount
        const amount = parseFloat(amountArg ?? '');
        if (isNaN(amount) || amount < constants_1.MIN_DEPOSIT_SOL || amount > constants_1.MAX_DEPOSIT_SOL) {
            throw errors_1.Errors.invalidAmount(amount, constants_1.MIN_DEPOSIT_SOL, constants_1.MAX_DEPOSIT_SOL);
        }
        ...
        // Dry run
        if (opts.dryRun) {
            const result = await (0, engine_1.dryRun)(side, amount, opts.priorityFee);
            (0, output_1.outputDryRun)(result);
            return;
        }
        // Play
        const result = await (0, engine_1.play)(side, amount, {
