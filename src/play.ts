...
import { MIN_DEPOSIT_SOL, MAX_DEPOSIT_SOL } from './constants';
...
`)
    .action(async (sideArg: string | undefined, amountArg: string | undefined, opts: Record<string, unknown>) => {
      ...
      // Dry run
      if (opts.dryRun) {
        const result = await dryRun(side, amount, opts.priorityFee as number | undefined);
        outputDryRun(result);
        return;
      }

      // Play
      const result = await play(side, amount, {
