 ...
 */
async function buildDepositTransaction(id, amount, side, priorityFee) {
    ...
    (0, logger_1.verboseLog)(`Building participate tx: ${amount} SOL, side ${side}`);
    try {
        ...
        (0, logger_1.verboseLog)(`Priority fee: ${fee} SOL`);
...
async function depositSol(id, amount, side, priorityFee) {
    try {
        ...
        const walletAdapter = (0, anchor_2.getWalletAdapter)();
        if (walletAdapter) {
            // Browser wallet adapter path
            const conn = (0, anchor_2.getConnection)();
            ...
            (0, logger_1.verboseLog)(`Deposit tx: ${signature}`);
            return signature;
        }
        else {
